  ...
  <div class="container" style="background-color: #F2F2F2;">
    ...
    <div class="row" >
      <div class="col-md-8" style="padding-right:0px">
        ...
        <div class="fly-panel" style="margin-top:10px">
        <div class="fly-panel-title fly-filter">
          <a>置顶</a>
          <a href="#signin" class="layui-hide-sm layui-show-xs-block fly-right" id="LAY_goSignin" style="color: #FF5722;">去签到</a>
        </div>
        <ul class="fly-list" id="dotop">     
        </ul>
      </div>
       ...
       <div class="fly-panel" style="margin-bottom: 0;">
        ...
        <div class="fly-panel-title fly-filter">
          <a href="action?url=WEB-INF/page/plate.jsp&forum=<%=new String(request.getParameter("forum").getBytes("ISO-8859-1"),"UTF-8")%>" class="layui-this">按时间</a>
          <span class="fly-mid"></span>
          <a href="javascript:void(0)" onclick="postbyhot()" id="postbyhot">按热议</a>
...
<script type="text/javascript">
  ...
  flow.load({
    ...
    ,done: function(page, next){ //到达临界点（默认滚动触发），触发下一页
      var lis = [];
      //以jQuery的Ajax请求为例，请求下一页数据（注意：page是从2开始返回）
      $.post('Preloads?method=getTopf&forum=<%=new String(request.getParameter("forum").getBytes("ISO-8859-1"),"UTF-8")%>&page='+page, function(res){
        //假设你的列表返回在data集合中      
        layui.each(res.data, function(index, item){
          console.log(item);
          lis.push('<li style="height:5%"><a href="action?url=WEB-INF/page/seeuser.jsp&uid='+item.uid+'" class="fly-avatar"><img src="UpAdown?method=uidheadimg&uid='+item.uid+'" alt=""></a><h2><a class="layui-badge">热门</a><a href="UserServlet?method=detailpost&pid='+item.pid+'&where=detail">'+item.subjects+'</a></h2><div class="fly-list-badge"></div><div class="fly-list-info"><a href="action?url=WEB-INF/page/seeuser.jsp&uid='+item.uid+'" link><cite>'+item.nick+'</cite></a><span>'+item.date+'</span><i class="layui-icon" title="点赞数">&#xe6c6 '+item.like+'</i></div><div class="fly-list-badge"><span class="layui-badge layui-bg-black">置顶</span></div></li>');
        }); 
...
function postbyhot(){
	$('#postbyhot').addClass('layui-this').siblings().removeClass('layui-this');
	  layui.use('flow', function(){
		  var $ = layui.jquery; //不用额外加载jQuery，flow模块本身是有依赖jQuery的，直接用即可。
		  var flow = layui.flow;
		  flow.load({
		    ...
		    ,done: function(page, next){ //到达临界点（默认滚动触发），触发下一页
		      var lis = [];
		      //以jQuery的Ajax请求为例，请求下一页数据（注意：page是从2开始返回）
		      $.post('Preloads?method=postbyhot&forum=<%=new String(request.getParameter("forum").getBytes("ISO-8859-1"),"UTF-8")%>&page='+page, function(res){
		        //假设你的列表返回在data集合中
